function [SDR,SIR,SAR,perm, SDR_block,SIR_block,SAR_block]=bss_eval_sources_shorttime(se,s)
...
% 2019-06-05 MGu, modified to include local performance measures;
% see Section II-C ("Local performance measures") in original paper at
...
SIR=zeros(nsrc,nsrc);
SAR=zeros(nsrc,nsrc);
SDR_block=cell(nsrc,nsrc);
SIR_block=cell(nsrc,nsrc);
SAR_block=cell(nsrc,nsrc);

for jest=1:nsrc,
    for jtrue=1:nsrc,
        [s_true,e_spat,e_interf,e_artif]=bss_decomp_mtifilt(se(jest,:),s,jtrue,512);
        [SDR(jest,jtrue),SIR(jest,jtrue),SAR(jest,jtrue), ...
            SDR_block{jest,jtrue},SIR_block{jest,jtrue},SAR_block{jest,jtrue}] ...
            =bss_source_crit(s_true,e_spat,e_interf,e_artif,512);
    end
end
...
SDR_block=SDR_block((0:nsrc-1).'*nsrc+perm);
SIR_block=SIR_block((0:nsrc-1).'*nsrc+perm);
SAR_block=SAR_block((0:nsrc-1).'*nsrc+perm);
return;
...
function [SDR,SIR,SAR, SDR_block,SIR_block,SAR_block]=bss_source_crit(s_true,e_spat,e_interf,e_artif,blockLength)
...
%%% Energy ratios short-time %%%
nBlock = floor(nsamplt/blockLength);
SDR_block = zeros(1,nBlock);
SIR_block = zeros(1,nBlock);
SAR_block = zeros(1,nBlock);
for kBlock = 1:nBlock
    idx = (1:blockLength) + (kBlock-1)*blockLength;
    % SDR
    SDR_block(kBlock)=10*log10(sum(sum(s_filt(:,idx).^2))/sum(sum((e_interf(:,idx)+e_artif(:,idx)).^2)));
    % SIR
    SIR_block(kBlock)=10*log10(sum(sum(s_filt(:,idx).^2))/sum(sum(e_interf(:,idx).^2)));
    % SAR
    SAR_block(kBlock)=10*log10(sum(sum((s_filt(:,idx)+e_interf(:,idx)).^2))/sum(sum(e_artif(:,idx).^2)));
end
